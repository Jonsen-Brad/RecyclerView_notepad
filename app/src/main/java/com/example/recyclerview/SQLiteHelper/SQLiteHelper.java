...
//        contentValues.put(DBUtils.NOTEPAD_TITLE, content);
     ...
     * @return
     */
    public List<NotepadBean> query(String Id) {
        List<NotepadBean> list = new ArrayList<NotepadBean>();
        Cursor cursor;
        if (Id.equals("")) {
            cursor = sqLiteDatabase.query(DBUtils.DATABASE_TABLE, null, null,
                    null, null, null, DBUtils.NOTEPAD_ID + " desc");
            //最后为id降序排列
        } else {
            cursor = sqLiteDatabase.query(DBUtils.DATABASE_TABLE, null, null,
                    null, null, null, DBUtils.NOTEPAD_ID + "<>" + Id + ","+DBUtils.NOTEPAD_ID+" desc");
        }
        if (cursor != null) {
            //循环将所有数据转入list
            while (cursor.moveToNext()) {
                ...
                //noteInfo.setNotepadContent(title);
    ...
    public List<NotepadBean> searchQuery(String string) {
        List<NotepadBean> list = new ArrayList<NotepadBean>();
        string = "%" + string + "%";
        ...
        if(string.equals("")){
            ...
            cursor = sqLiteDatabase.query(DBUtils.DATABASE_TABLE, null, DBUtils.NOTEPAD_CONTENT + " like ?",
